...
```bash
random-letter-sam$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
random-letter-sam$ sam local start-api
random-letter-sam$ curl http://localhost:3000/
```
...
```bash
random-letter-sam$ sam logs -n HelloWorldFunction --stack-name random-letter-sam --tail
```
...
```bash
random-letter-sam$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name random-letter-sam
```
